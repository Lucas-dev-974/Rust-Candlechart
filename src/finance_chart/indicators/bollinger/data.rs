...
/// Calcule la plage de prix (min, max) pour les bandes de Bollinger visibles
///
/// # Arguments
/// * `visible_bollinger_values` - Les valeurs Bollinger visibles
///
/// # Retourne
/// `Some((min, max))` si des valeurs valides existent, `None` sinon
pub fn calculate_bollinger_range(
    visible_bollinger_values: &[Option<BollingerValue>],
) -> Option<(f64, f64)> {
    let (min, max) = visible_bollinger_values
        .iter()
        .filter_map(|opt| opt.as_ref())
        .fold((f64::INFINITY, f64::NEG_INFINITY), |(min, max), bb| {
            let min_val = min.min(bb.lower).min(bb.middle);
            let max_val = max.max(bb.upper).max(bb.middle);
            (min_val, max_val)
        });
...
/// Récupère la dernière valeur Bollinger valide
///
/// # Arguments
/// * `chart_state` - L'état du graphique (utilisé si all_bollinger_values est None)
/// * `all_bollinger_values` - Toutes les valeurs Bollinger pré-calculées (optionnel)
///
/// # Retourne
/// La dernière valeur Bollinger valide, ou `None` si aucune n'est disponible
pub fn get_last_bollinger_value(
    chart_state: &ChartState,
    all_bollinger_values: Option<&Vec<Option<BollingerValue>>>,
) -> Option<BollingerValue> {
    // Utiliser les valeurs pré-calculées si disponibles
    if let Some(values) = all_bollinger_values {
    ...
    chart_state
        ...
        .and_then(|all_candles| {
            if all_candles.len() < BOLLINGER_PERIOD {
                return None;
            }
            let bollinger_values = calculate_bollinger_bands(
                all_candles,
                BOLLINGER_PERIOD,
                BOLLINGER_STD_DEV,
            );
            bollinger_values.iter().rev().find_map(|opt| opt.clone())
        })
}
